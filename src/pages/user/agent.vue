    ...
    <Modal v-model="dialogShow" :title="label[currDialog]" :mask-closable="false" @on-cancel="resetDialogForm('formDialog')">
      <Form :model="formDialog" ref="formDialog" :rules="rules" :label-width="80">
        ...
        <Row v-if="formDialog.agentType!=0">
          <Col span="12">
            <FormItem label="拥有账号" prop="isUserApp">
              <Select v-model="formDialog.isUserApp" placeholder="请选择" transfer>
                <Option v-for="item in isUserApp" :key="item.value" :value="item.value">{{item.label}}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="12" v-if="formDialog.isUserApp==1">
            <FormItem label="账号昵称" prop="userId">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            'key': 'isUserApp',
            render: (create, params) => {
              var key=params.row.isUserApp,map=this.isUserAppMap
    ...
    watch: {
      ...
      'formDialog.isUserApp': function(val){
