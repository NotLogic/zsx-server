  ...
  <div class="chicken-soup">
    ...
    <Modal v-model="dialogShow" :title="label[currDialog]" :mask-closable="false" @on-cancel="resetDialogForm('formDialog')">
      <Form :model="formDialog" ref="formDialog" :rules="rules" :label-width="80">
        <Row>
          <Col span="12">
            <FormItem label="心语分类" prop="classId">
              <Select v-model="formDialog.classId" placeholder="请选择"  clearable>
                <Option v-for="item in classId" :value="item.value" :key="item.value">{{ item.label }}</Option>
              </Select>
            </FormItem>
          </Col>
        </Row>
        <Row>
          <Col span="24">
            <FormItem label="心语内容" prop="content">
              <Input v-model="formDialog.content" type="textarea" :rows="7" placeholder="请输入心语内容"></Input>
            </FormItem>
          </Col>
        </Row>
      </Form>
  ...
  export default {
    ...
    data(){
      return {
        ...
        rules: {
          classId: [
            // { required: true, message: '心语分类不能为空', trigger: 'change' },
          ],
          content: [
